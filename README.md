...
## Download 
All installers linked here are compatible with **Windows 10** and **Windows 11**. To run on any other operating system with Python 3.12 or above, see the "Runing with Python" instructions below. 
...
### Running with python
This software can be run on any operating system with Python. This is more involved than using one of the above Windows installers. It requires python 3.12 or above. Recommended avoiding virtual machines to avoid headaches with usb communications and port access. But this software is free to use, so be free to make your own decisions :) To run with python, follow these steps:
